...
def save_all_plots(teacher, student, alignment, metrics, img_dir: Path) -> dict[str, str]:
    ...
    _plot_logmel(student, teacher, "Ученик", out("logmel_student.png"), pair_for_range=True)
...
def _plot_pitch_contours(teacher, student, path: Path) -> None:
    ...
    ax.plot(teacher["time"], t_f0, color=TEACHER_COLOR, lw=1.6,
            label=f"{TEACHER_LABEL} F0", zorder=3)
    ax.plot(student["time"], s_f0, color=STUDENT_COLOR, lw=1.4, alpha=0.9,
            label=f"{STUDENT_LABEL} F0", zorder=3)

    # Онсеты как маркеры по краям оси (не вертикальные стены)
    trans = ax.get_xaxis_transform()
    if len(teacher["onsets"]) > 0:
        ax.plot(teacher["onsets"], np.full_like(teacher["onsets"], 1.0),
                marker="v", linestyle="", color=TEACHER_COLOR,
                markersize=7, transform=trans, clip_on=False,
                label=f"Онсеты ({TEACHER_LABEL.lower()})")
    if len(student["onsets"]) > 0:
        ax.plot(student["onsets"], np.full_like(student["onsets"], 0.0),
                marker="^", linestyle="", color=STUDENT_COLOR,
                markersize=7, transform=trans, clip_on=False,
    ...
    ax.set_yticks(ticks)
    ax.set_yticklabels([f"{name} ({int(round(hz))} Гц)" for name, hz in zip(labels, ticks)])
    ax.minorticks_off()
...
def _add_cents_zones(ax, x_label_pos: float = 0.985):
    ...
    for y, color in [(25, GOOD_COLOR), (-25, GOOD_COLOR),
                     (50, OKAY_COLOR), (-50, OKAY_COLOR),
                     (100, BAD_COLOR), (-100, BAD_COLOR)]:
        ax.axhline(y, color=color, ls="--", lw=0.8, alpha=0.7, zorder=1)

    label_kwargs = dict(transform=ax.get_yaxis_transform(),
                        ha="right", va="center", fontsize=9, fontweight="bold")
    ax.text(x_label_pos, 0, "Хорошо ±25¢", color="#1F7E4F",
            bbox=dict(facecolor="white", edgecolor=GOOD_COLOR, boxstyle="round,pad=0.25", alpha=0.85),
            **label_kwargs)
    ax.text(x_label_pos, 37.5, "Приемлемо ±50¢", color="#9F6B17",
            bbox=dict(facecolor="white", edgecolor=OKAY_COLOR, boxstyle="round,pad=0.25", alpha=0.85),
            **label_kwargs)
    ax.text(x_label_pos, 75, "Плохо ±100¢", color="#9F2C2C",
            bbox=dict(facecolor="white", edgecolor=BAD_COLOR, boxstyle="round,pad=0.25", alpha=0.85),
            **label_kwargs)
...
def _plot_pitch_error(teacher, alignment, path: Path) -> None:
    ...
    if err.size and alignment["teacher_idx"].size:
        ...
        ax.axhline(mean_e, color=ps.ACCENT_COLOR, ls="--", lw=1.0, alpha=0.85,
                   label=f"Среднее = {mean_e:+.1f}¢")
        ax.axhline(median_e, color="#3A3A45", ls=":", lw=1.0, alpha=0.85,
        ...
        ax.text(
            ...
            fontsize=10, fontweight="bold", color=ps.TEXT_MUTED,
            bbox=dict(facecolor="white", edgecolor=ps.NEUTRAL_GRID,
...
def _plot_pitch_error_hist(alignment, metrics, path: Path) -> None:
    ...
    if err.size:
        ...
        ax.hist(clipped, bins=50, color=ps.ACCENT_COLOR, alpha=0.78,
        ...
        ax.axvline(median, color="#3A3A45", ls=":", lw=1.4,
                   label=f"Медиана = {median:+.1f}¢", zorder=3)
        ax.axvline(mae, color=BAD_COLOR, ls="--", lw=1.4,
                   label=f"MAE = {mae:.1f}¢", zorder=3)
        ...
        ax.text(
            ...
            fontsize=9.5, fontweight="bold", color=ps.TEXT_MUTED,
            bbox=dict(facecolor="white", edgecolor=ps.NEUTRAL_GRID,
                      boxstyle="round,pad=0.35"),
        )
        ax.legend(loc="upper left")
...
def _plot_logmel(primary, other, role: str, path: Path, pair_for_range: bool = False) -> None:
    ...
    img = ax.imshow(
        primary["logmel"], aspect="auto", origin="lower",
...
def _plot_ltas(teacher, student, path: Path) -> None:
    ...
    ax.axvspan(2500, 3500, alpha=0.10, color="#2EA56A",
               label="Singer's formant (2.5–3.5 кГц)")
    ax.axvspan(1000, 5000, alpha=0.06, color="#E0A53B",
               label="Alpha ratio: верхняя полоса (1–5 кГц)")

    ax.plot(f_t[valid_t], l_t[valid_t], color=TEACHER_COLOR, lw=1.5,
            label=f"{TEACHER_LABEL}")
    ax.plot(f_s[valid_s], l_s[valid_s], color=STUDENT_COLOR, lw=1.4, alpha=0.9,
...
def _plot_formants(teacher, student, path: Path) -> None:
    fig, ax = plt.subplots(figsize=(14, 4))
    ax.plot(teacher["time"], voiced_only(teacher["f1"]),
            color=TEACHER_COLOR, lw=1.2, label=f"{TEACHER_LABEL} F1")
    ax.plot(teacher["time"], voiced_only(teacher["f2"]),
            color=TEACHER_COLOR, lw=1.2, ls="--", alpha=0.85, label=f"{TEACHER_LABEL} F2")
    ax.plot(student["time"], voiced_only(student["f1"]),
            color=STUDENT_COLOR, lw=1.2, alpha=0.95, label=f"{STUDENT_LABEL} F1")
    ax.plot(student["time"], voiced_only(student["f2"]),
            color=STUDENT_COLOR, lw=1.2, ls="--", alpha=0.85, label=f"{STUDENT_LABEL} F2")
...
def _plot_intensity_dynamics(teacher, student, path: Path) -> None:
    fig, ax = plt.subplots(figsize=(14, 4))
    ax.plot(teacher["time"], teacher["intensity"], color=TEACHER_COLOR, lw=1.4,
            label=f"{TEACHER_LABEL} — интенсивность")
    ax.plot(student["time"], student["intensity"], color=STUDENT_COLOR, lw=1.3,
            alpha=0.9, label=f"{STUDENT_LABEL} — интенсивность")

    ax2 = ax.twinx()
    ax2.plot(teacher["time"], teacher["rms"], color=TEACHER_COLOR, lw=0.9,
             alpha=0.45, ls=":", label=f"{TEACHER_LABEL} — RMS")
    ax2.plot(student["time"], student["rms"], color=STUDENT_COLOR, lw=0.9,
             alpha=0.45, ls=":", label=f"{STUDENT_LABEL} — RMS")
...
def _plot_cpp_alpha(teacher, student, path: Path) -> None:
    ...
    ax_cpp.plot(teacher["time"], t_cpp, color=TEACHER_COLOR, lw=1.4,
                label=f"{TEACHER_LABEL} CPP")
    ax_cpp.plot(student["time"], s_cpp, color=STUDENT_COLOR, lw=1.3, alpha=0.9,
                label=f"{STUDENT_LABEL} CPP")
    ax_cpp.axhline(15, color=OKAY_COLOR, ls="--", lw=1.0,
                   label="15 дБ — порог чёткости")
    ax_cpp.axhline(20, color=GOOD_COLOR, ls="--", lw=1.0,
                   label="20 дБ — чистый голос")
    ax_cpp.text(0.985, 0.05, "ниже — придыхательный  /  выше — чёткий",
                transform=ax_cpp.transAxes, ha="right", va="bottom",
                fontsize=9, color=ps.TEXT_MUTED, style="italic")
    ...
    t_alpha = np.interp(teacher["time"], teacher["alpha_times"], teacher["alpha_contour"])
    s_alpha = np.interp(student["time"], student["alpha_times"], student["alpha_contour"])
    ax_alpha.plot(teacher["time"], t_alpha, color=TEACHER_COLOR, lw=1.4,
                  label=f"{TEACHER_LABEL} Alpha ratio")
    ax_alpha.plot(student["time"], s_alpha, color=STUDENT_COLOR, lw=1.3, alpha=0.9,
                  label=f"{STUDENT_LABEL} Alpha ratio")
    ax_alpha.text(0.985, 0.05, "выше — twang / ярче  /  ниже — sob / темнее",
...
def _plot_scores_summary(metrics, path: Path) -> None:
    ...
    bars = ax.barh(y_pos, plot_vals, color=colors, edgecolor="white",
                   height=0.65, zorder=2)

    for bar, val in zip(bars, values):
        if np.isfinite(val):
            ax.text(min(val + 1.5, 100.5), bar.get_y() + bar.get_height() / 2,
                    f"{val:.1f}", va="center", ha="left",
                    fontsize=11, fontweight="bold", color="#2A2A30")
        else:
            ax.text(2, bar.get_y() + bar.get_height() / 2, "n/a",
                    va="center", ha="left", fontsize=10, color=ps.TEXT_MUTED)
    ...
    ax.text(60, len(labels) - 0.3, "≥60 приемлемо", color="#9F6B17",
            fontsize=8.5, ha="center", fontweight="bold")
    ax.text(80, len(labels) - 0.3, "≥80 хорошо", color="#1F7E4F",
            fontsize=8.5, ha="center", fontweight="bold")
